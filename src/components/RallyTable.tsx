        ...
        <div className="container-fluid mt-4">
            ...
            <div className="d-flex flex-column flex-sm-row justify-content-center align-items-center mb-4 w-100">
                <div className="mb-2 mb-sm-0 me-sm-2">
                    <label htmlFor="vista-select" className="form-label me-2 mb-0">Modo:</label>
                    <select
                        id="vista-select"
                ...
                <div className="mb-2 mb-sm-0">
                    <label htmlFor="pasada-select" className="form-label me-2 mb-0">Ver:</label>
                    <select
                        id="pasada-select"
                        className="form-select"
            ...
            <div className="table-responsive-sm rounded-3 shadow">
                <table className="table table-striped table-hover mb-0">
                    <thead className="bg-light">
                        <tr>
                            <th scope="col" className="text-nowrap">Piloto</th>
                            {selectedPasada === null ? (
                                <>
                                    {pasadasDisponibles.map((pasada) => (
                                        <th key={pasada.id} scope="col" className="text-center text-nowrap">
                                            {pasada.nombre}
                                        </th>
                                    ))}
                                    <th scope="col" className="text-center text-nowrap">Total</th>
                                </>
                            ) : (
                                <>
                                    {pilotos[0].pasadas[selectedPasada].tramos.map((_, index) => (
                                        <th key={index} scope="col" className="text-center text-nowrap">
                                            Tramo {index + 1}
                                        </th>
                                    ))}
                                    <th scope="col" className="text-center text-nowrap">Subtotal</th>
                                </>
                            )}
                            {modoVista === 'edicion' && (
