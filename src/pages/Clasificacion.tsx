...
export default function Clasificacion() {
  ...
  useEffect(() => {
    const fetchTiempos = async () => {
      // 0. Fetch Categorias completas (para calcular el Ángulo Dorado)
      const { data: catData } = await supabase.from('categories').select('id, name').order('id');
      if (catData) {
        setCategoriasLista(catData);
      }
